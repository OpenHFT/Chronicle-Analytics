...
=== CA-TEST-001: Maven worker isolation

Maven gives each test worker its own working directory and Java `user.home`
under `target/surefire-home-*`. This isolates client IDs and rate-limit files
from other workers and concurrently built repositories, so the frequency-reset
test does not skip because another JVM updated the same file. The existing
four-worker policy and fork reuse are retained. This applies only to test JVMs.
